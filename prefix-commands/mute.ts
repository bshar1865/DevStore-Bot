import {
  ...
  module.exports = {
    ...
    async execute(message: Message, args: string[]) {
      const requiredRoles = [idclass.roleFA];
      ...
      return message.reply('You can’t mute yourself. This isn’t a therapy session.');
      }

      if (member.id === message.client.user?.id) {
      return message.reply([
      'I’m not into self-sabotage, thanks.',
      'You want me to mute myself? Bold of you.',
      'Muting myself? I don’t think so.',
      'Nah, I run this place.',
      'Error: Bot refuses to commit digital suicide.',
      ][Math.floor(Math.random() * 5)]);
     }


      if (member.roles.highest.position >= message.guild!.members.me!.roles.highest.position) {
      return message.reply('I cannot mute someone with a higher or equal role than mine.');
      }
  
      const protectedRoles = [idclass.roleFA];
      if (member.roles.cache.some(role => protectedRoles.includes(role.id))) {
        const embed = new EmbedBuilder()
          .setColor('#FFA500')
          .setDescription('You cannot mute trusted/mods.');
